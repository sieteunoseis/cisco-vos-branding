...
def create_transparent_image_with_text(
    width,
    height,
    text,
    font_name,
...
def create_properties_file(color1, color2, color3, end_color, filename, is_root=False, style='gradient'):
    if is_root:
        properties_content = """#selfcare logo
selfcare.companyLogoImage = ../branding/branding_logo.png"""
    else:
        # Set header heading color based on style
        heading_color = "#FFFFFF" if style == "solid" else end_color
...
header.heading.color = {heading_color}
...
def main():
    ...
    parser.add_argument(
        "--glow", action="store_true", help="Add glow effect to the text"
    )
    parser.add_argument(
        "--start-color",
        default="#FFFFFF",
    ...
    parser.add_argument(
        "--application",
        choices=["ccmadmin", "cucadmin", "CER"],
        default="ccmadmin",
        help="Target application (ccmadmin, cucadmin, or CER). Default is ccmadmin",
    )
    parser.add_argument(
        "--text", default="AB", help="Text to use in the logos (default: AB)"
    )
    args = parser.parse_args()

    # Validate hex colors
    color_pattern = re.compile(r"^#[0-9A-Fa-f]{6}$")
    if not color_pattern.match(args.start_color) or not color_pattern.match(
        args.end_color
    ):
    ...
    app_path = os.path.join(branding_path, args.application)
    ensure_directory(branding_path)
    ensure_directory(app_path)

    # Determine pixel width based on application
    pixel_width = 652 if args.application == "CER" else 2048
    ...
    print(f"Target application: {args.application} (width: {pixel_width}px)")
    print(f"Logo text: {args.text}")
    ...
    create_properties_file(
        ...
        middle_color2,
        end_color,
        os.path.join(app_path, "BrandingProperties.properties"),
        is_root=False,
        style=args.style,
    )
    ...
    if args.style == "gradient":
        ...
        create_gradient(
            ...
            start_color,
            middle_color1,
            os.path.join(app_path, "brandingHeaderBegLTR.gif"),
            rtl=False,
        )
        create_gradient(
            652,
            middle_color1,
            middle_color2,
            os.path.join(app_path, "brandingHeaderMidLTR.gif"),
            rtl=False,
        )
        create_gradient(
            652,
            middle_color2,
            end_color,
            os.path.join(app_path, "brandingHeaderEndLTR.gif"),
            rtl=False,
        )
        ...
        create_gradient(
            ...
            start_color,
            middle_color1,
            os.path.join(app_path, "brandingHeaderBegRTL.gif"),
            rtl=True,
        )
        create_gradient(
            652,
            middle_color1,
            middle_color2,
            os.path.join(app_path, "brandingHeaderMidRTL.gif"),
            rtl=True,
        )
        create_gradient(
            652,
            middle_color2,
            end_color,
            os.path.join(app_path, "brandingHeaderEndRTL.gif"),
            rtl=True,
        )
    else:  # solid style
        # Create solid background image
        create_solid_background(
            pixel_width, 1, end_color, os.path.join(app_path, "brandingHeader.gif")
        )
    ...
    create_transparent_image_with_text(
        ...
        logo_color,
        0,
        os.path.join(app_path, "ciscoLogo12pxMargin.gif"),
        glow=args.glow,
        y_offset=0,
        x_offset=2,
        antialias=args.antialias,
